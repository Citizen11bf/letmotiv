...
document.querySelector('#calc')?.addEventListener('click',()=>{calc();const r=document.querySelector('.result');if(r){r.classList.remove('revealed');void r.offsetWidth;r.classList.add('revealed')}});calc();
document.querySelectorAll('.logos img').forEach(img=>img.addEventListener('click',()=>img.classList.toggle('active')));
